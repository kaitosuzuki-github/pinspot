...
RSpec.describe "Profiles", type: :request do
  ...
  describe 'GET /profiles/:id/show_bookmarks' do
    let(:user) { create(:user) }
    let(:post) { create(:post) }
    ...
    context '正常な場合' do
      ...
      it '正常なレスポンスを返すこと' do
        get show_bookmarks_profile_path(user.profile.id)
        expect(response).to have_http_status(:success)
      end
      ...
      it 'プロフィールに関連したブックマークをした投稿名を表示すること' do
        get show_bookmarks_profile_path(user.profile.id)
        expect(response.body).to include post.title
      end
    end
  ...
  describe 'GET /profiles/:id/followers' do
    ...
    context '正常な場合' do
      ...
      it '正常なレスポンスを返すこと' do
        get followers_profile_path(user.profile.id)
        expect(response).to have_http_status(:success)
      end

      it 'フォローワーのプロフィール名を表示すること' do
        get followers_profile_path(user.profile.id)
